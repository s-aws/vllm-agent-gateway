...
Phase 247 records the committed V1.1 ship-ready release-candidate state in metadata and validates that the tester-facing docs point at that state. Later 500k-specific proof is tracked by Phases 270 through 277 and should be used for current large-context tester instructions.
...
- Known limitations remain visible: this is not a production deployment, the current large-context target is usable 500k-token projects through governed context strategy, raw 500k or 1M-token prompt serving is not claimed, and advanced broad refactor orchestration is not released.
- AnythingLLM must point at the workflow-router gateway for natural workflow routing. Use `http://127.0.0.1:8500/v1` only when the AnythingLLM client can reliably read loopback response bodies; otherwise use the printed WSL network workflow-router URL.
...
Advanced broad refactor orchestration is not released. Raw 500k or 1M-token prompt serving is not claimed. The 384k-token project usability baseline remains preserved as lineage while stable testers use the governed 500k-token project-usability path. This is a local founder-testing release, not a production deployment.
