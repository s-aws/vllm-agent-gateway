...
AnythingLLM's API surface has a separate loopback conflict failure mode. If `http://127.0.0.1:3001/api/ping` returns HTML or `404` while `AnythingLLM.exe` is listening on `0.0.0.0:3001`, another local process may own the more-specific `127.0.0.1:3001` binding. Do not treat that as an auth or model failure. Probe the Windows/WSL network addresses, pass the working value to validators with `--anythingllm-api-base-url`, and keep the internal Bash workflow-router gateway on `http://127.0.0.1:8500/v1`. Phase 323 proved `http://192.168.0.208:3001` and `http://100.100.12.45:3001` as working AnythingLLM API bases on the current host, while `http://127.0.0.1:3001` was a different Node app.

AnythingLLM UI testing requires the workspace `chatMode` to be `chat`. The current AnythingLLM `/stream-chat` endpoint invokes AnythingLLM's agent layer when the workspace is in `automatic` mode, which returns `@agent` session messages instead of the workflow-router answer. Treat `automatic` chat mode as an AnythingLLM setup/configuration failure for browser-visible workflow-router testing, not as a local-model or gateway failure.

The product objective includes large-context usability, not a raw context-size guarantee. Governed 500k-token project usability is the current stable tester path after the Phase 270-277 proof chain. The 384k-token projects usability baseline remains preserved as lineage. Historical Phase 270 lineage still uses the terms `500k-token project usability candidate` and `384k stable large-context baseline`; keep those markers for replayability even though the governed 500k path is now stable. Do not assume the current local model can accept raw 500k-token prompts unless a dedicated proof gate validates model config, vLLM settings, hardware memory, latency, and blind-baseline answer quality.
