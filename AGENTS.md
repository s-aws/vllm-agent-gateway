...
The product objective includes large-context usability, not a raw context-size guarantee. The 384k stable large-context baseline for 384k-token projects is complete and remains the current stable tester path. A 500k-token project usability candidate is approved under the post-384k expansion gate, but it must not be treated as stable until it has dedicated fixture, safety, live gateway, AnythingLLM, clean-clone, decision, and handoff proof. Do not assume the current local model can accept raw 500k-token prompts unless a dedicated proof gate validates model config, vLLM settings, hardware memory, latency, and blind-baseline answer quality.
