...
The product objective includes large-context usability, not a raw context-size guarantee. Governed 500k-token project usability is the current stable tester path after the Phase 270-277 proof chain. The 384k-token project usability baseline remains preserved as lineage. Do not assume the current local model can accept raw 500k-token prompts unless a dedicated proof gate validates model config, vLLM settings, hardware memory, latency, and blind-baseline answer quality.
