...
Large-context support currently means making 384k-token projects usable through indexing, retrieval, chunking, summarization, artifact paging, evidence selection, and model-context-aware routing. It does not mean promising that the current local model can accept raw 384k-token prompts. Work above 384k tokens, including 1M+ project usability, is paused until the 384k product path has a stable usable tester handoff and a future milestone is explicitly approved.
