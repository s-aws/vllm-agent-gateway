...
- Status: failed
- Created at: 20260606T021045154243Z
- Prompt count: 34
- Passed: 30
- Failed: 4
...
| P01-V1 | refined | failed | code_investigation.plan | code_investigation.plan | l1_find_behavior_start_terms | l1_find_related_tests_terms | workflow matched but classifier explainability rule did not |
| P08-V1 | refined | failed | code_investigation.plan | code_investigation.plan | l1_endpoint_route_lookup_terms | l1_find_related_tests_terms | workflow matched but classifier explainability rule did not |
...
| P21-V1 | refined | failed | code_investigation.plan |  | l2_change_surface_summary_terms | no_supported_workflow_match |  |
| P23-V1 | refined | failed | execution_planning.plan |  | l1_small_text_edit_terms | no_supported_workflow_match |  |
...
Note: Clarifies that start means the first source point that creates or populates the lookup key.
...
Note: Prevents stopping at a UI sender instead of following the handler branch.
...
Note: Distinguishes database schema from related runtime dictionary fields.
...
Note: Avoids confusing the trading-engine entrypoint with a dashboard service.
...
Note: Names the shell surface while preserving the test-selection intent.
...
Note: Makes the change-surface boundary explicit.

Rules: `no_supported_workflow_match`
...
Note: Requests a tighter review surface for the draft documentation edit.
...
Note: Uses older-client-safe copy-only approval wording.
