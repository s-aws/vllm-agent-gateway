...
| P0-M15-275 | clean-clone 500k candidate replay | Approved for Phase 275. Replay the 500k candidate path from a fresh remote branch clone without active workspace state. | Required proof: Phase 275 policy, validator, docs, remote clone source checks, controller preflight proving the live stack is clone-hosted, Phase 270 through Phase 274 replay, live gateway, live AnythingLLM, clone clean before/after, focused regression, docs index, and clean-clone proof. |
...
- `P0-M15-275`: clean-clone 500k candidate replay. Approved; live clone-hosted proof remains required.
...
`P0-M15-275` is approved for Phase 275. It replays Phase 270 through Phase 274 from a fresh remote branch clone and requires the clone to remain clean before and after validation with `runtime-state/` ignored. The initial live replay attempt exposed an environment preflight gap: the running controller was hosted from `/mnt/c/agentic_agents`, not the clone, so the clean-clone proof could not truthfully close. Phase 275 now requires controller preflight proof that the live stack is clone-hosted before the downstream live gates can satisfy the phase. It does not promote 500k to stable; Phase 276 decision remains required after clean-clone proof passes.
