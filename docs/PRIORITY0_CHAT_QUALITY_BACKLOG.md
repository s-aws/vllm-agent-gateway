...
Priority 0 also covers large-context usability when it improves that chat outcome. The target is that very large repositories and corpora, including 1M+ token projects, can be investigated through indexing, retrieval, chunking, summarization, artifact paging, evidence selection, and model-context-aware routing. Raw 1M-token prompts are not considered supported until a separate proof gate validates the model, vLLM configuration, hardware memory, latency, and blind-baseline answer quality.
...
| P0-M14-238 | Release-candidate PR readiness packet | Approved in Phase 238. Make the pushed release-candidate branch reviewable without private chat context. | Required proof: branch scope hygiene, no generated runtime-state/protected fixture files, proof summary, known limits, test commands, docs links, rollback path, and draft/PR readiness state if a PR is created. |
| P0-M14-239 | Remote-clone Priority 0 chat-quality replay | Approved in Phase 239. Re-run representative chat-quality prompts from the remote-clone path using blind-baseline comparison. | Required proof: blind baselines, gateway and AnythingLLM responses, route/skill/tool evidence, scoring, holdout reruns after any repair, no unresolved high/critical gaps, and fixture mutation proof. |
| P0-M14-240 | Remote-clone non-Coinbase generalization replay | Approved in Phase 240. Confirm release-candidate behavior still generalizes beyond Coinbase from the clone path. | Required proof: Python-service and Staterail approved fixture prompts, allowed-root evidence, gateway/AnythingLLM proof where runtime-facing, no commits/pushes to fixture repositories, and mutation proof. |
| P0-M14-241 | Large-context release-candidate strategy replay | Approved in Phase 241. Confirm large-context usability and safety behavior from the release-candidate path. | Required proof: retrieval/artifact paging/summarization/refusal/chunked-investigation coverage where supported, index safety, source hash freshness, no raw 1M claim, AnythingLLM representative proof, and no artifact-only answers. |
| P0-M14-242 | Release-candidate baseline corpus promotion | Approved in Phase 242. Promote passing release-candidate prompt coverage into governed baseline corpus. | Required proof: prompt hashes, blind-baseline records, expected route/workflow/skill/tool metadata, answer markers, holdouts, no unresolved high/critical gaps, and deterministic corpus validation. |
| P0-M14-243 | External tester feedback loop from clone | Approved in Phase 243. Prove release-candidate tester feedback becomes traceable work. | Required proof: positive and missing/defect feedback records linked to target run IDs, accepted/rejected/deferred/advisory/blocking classification, rerun requirement for accepted fixes, and ignored local artifacts. |
| P0-M14-244 | V1 release-candidate decision gate | Approved in Phase 244. Aggregate release-candidate proof into ship, hold, or repair-required decision. | Required proof: Phase 232-243 result aggregation, run IDs, docs/regression/fixture proof, known limits, unresolved finding list, deterministic decision, and next phase batch only if repair is required. |
...
Phase 237 AnythingLLM Fresh Chat Responsiveness Rebaseline is complete. Phase 238 Release-Candidate PR Readiness Packet is the active next Priority 0 action. Do not begin advanced refactor work unless the canonical roadmap explicitly marks that phase active.
