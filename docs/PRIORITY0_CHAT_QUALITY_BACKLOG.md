...
| P0-M14-236 | Remote branch clone release proof | Approved in Phase 236. Prove the current release candidate can be committed, pushed, cloned, started, and tested from a remote branch without active workspace state. | Required proof: clean branch commit scope, no generated runtime-state or protected fixture commit, remote branch push, disposable clone, clone-safe routing gate, first-time doctor, docs index, clean handoff proof from clone, live AnythingLLM onboarding, fixture mutation proof, and full regression as blast radius requires. |
...
Phase 235 Clone-Safe Model Capability Routing is complete. Phase 236 Remote Branch Clone Release Proof is the active next Priority 0 action. Do not begin advanced refactor work unless the canonical roadmap explicitly marks that phase active.
