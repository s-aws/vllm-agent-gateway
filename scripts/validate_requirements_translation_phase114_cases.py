#!/usr/bin/env python3
"""Validate the Phase 114 requirements translation prompt and audit case catalog."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--catalog-path", default=str(DEFAULT_PHASE114_CASE_CATALOG))
    parser.add_argument("--output-path", default=None)
    parser.add_argument("--recursive-report-output-path", default=None)
    parser.add_argument(
        "--live-report-path",
        default="runtime-state/task-decomposition/phase114-requirements-live.json",
        help="Evidence reference for the Phase 114 live validation report.",
    )
    parser.add_argument(
        "--engineering-tenet-report-path",
        default="runtime-state/engineering-tenet-coverage/phase114-current.json",
        help="Evidence reference for the engineering-tenet coverage report.",
    )
    parser.add_argument(
        "--focused-regression-ref",
        default="python -m pytest tests/regression/test_task_decomposition.py -q -> 27 passed",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="runtime-state/recursive-blind-testing/phase114-requirements-translation-recursive-validation.json",
    )
    parser.add_argument("--final-audit-ref", default="contextless subagent follow-up audit -> score 88/100")
    parser.add_argument("--final-audit-score", type=int, default=88)
...
def main() -> int:
    ...
    if args.recursive_report_output_path:
        ...
        recursive_report = build_phase114_recursive_blind_testing_report(
            catalog,
    ...
    print("REQUIREMENTS TRANSLATION PHASE114 CASES " + json.dumps(report, ensure_ascii=True, sort_keys=True))
