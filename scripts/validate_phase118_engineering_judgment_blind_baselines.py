#!/usr/bin/env python3
"""Validate Phase 118 engineering-judgment blind baselines."""
...
def validate_baselines(cases_catalog: dict[str, Any], baselines_catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    if baselines_catalog.get("kind") != "phase118_engineering_judgment_blind_baselines":
        issues.append(issue("catalog", "kind must be phase118_engineering_judgment_blind_baselines"))
    if baselines_catalog.get("schema_version") != 1:
        issues.append(issue("catalog", "schema_version must be 1"))
    if baselines_catalog.get("phase") != 118:
        issues.append(issue("catalog", "phase must be 118"))
    if baselines_catalog.get("priority_backlog_id") != "P0-BB-003":
        issues.append(issue("catalog", "priority_backlog_id must be P0-BB-003"))
    ...
    blind_summary = baselines_catalog.get("blind_agent_summary")
    if not isinstance(blind_summary, dict):
        issues.append(issue("blind_agent_summary", "blind_agent_summary must record contextless evaluator expectations"))
    else:
        fields = non_empty_string_list(blind_summary.get("must_have_fields"))
        hard_fails = non_empty_string_list(blind_summary.get("hard_fail_conditions"))
        if len(fields) < 6:
            issues.append(issue("blind_agent_summary", "must_have_fields must include at least six fields"))
        if len(hard_fails) < 3:
            issues.append(issue("blind_agent_summary", "hard_fail_conditions must include at least three entries"))
    ...
    for case_id in case_ids:
        ...
        if missing_keys:
            issues.append(issue(str(case_id), f"missing baseline keys: {missing_keys}"))
        for key in ("must_have_facts", "evidence_expectations", "safety_boundaries", "likely_local_model_failure_modes"):
            ...
            if len(values) < 3:
                issues.append(issue(str(case_id), f"{key} must include at least three non-empty strings"))
        ideal = baseline.get("ideal_answer_shape")
        if not isinstance(ideal, str) or len(ideal.split()) < 12:
            issues.append(issue(str(case_id), "ideal_answer_shape must be a useful sentence"))
        suggestion = baseline.get("prompt_tightening_suggestion")
        if not isinstance(suggestion, str) or len(suggestion.split()) < 8:
            issues.append(issue(str(case_id), "prompt_tightening_suggestion must be a useful sentence"))
        rubric = baseline.get("scoring_rubric_100")
        if not isinstance(rubric, dict) or not rubric:
            issues.append(issue(str(case_id), "scoring_rubric_100 must be a non-empty object"))
        else:
            total = sum(value for value in rubric.values() if isinstance(value, int))
            if total != 100:
                issues.append(issue(str(case_id), f"scoring_rubric_100 must total 100, got {total}"))
            ...
            if missing_topics:
                issues.append(issue(str(case_id), f"scoring_rubric_100 missing rubric topics: {missing_topics}"))

    return {
        "kind": "phase118_engineering_judgment_blind_baseline_report",
        "schema_version": 1,
...
def main() -> int:
    ...
    print("PHASE118 ENGINEERING JUDGMENT BLIND BASELINES " + json.dumps(report, ensure_ascii=True, sort_keys=True))
