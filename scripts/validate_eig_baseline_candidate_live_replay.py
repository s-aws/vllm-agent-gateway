#!/usr/bin/env python3
"""Validate the Phase 308 live replay for EIG baseline candidates."""
...
from vllm_agent_gateway.acceptance.eig_baseline_candidate_live_replay import (  # noqa: E402
    DEFAULT_POLICY_PATH,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-path",
        default="runtime-state/eig-baseline-candidate-live-replay/phase308-validation.json",
    )
...
def main() -> int:
    ...
    print("EIG BASELINE CANDIDATE LIVE REPLAY REPORT " + str(args.output_path))
    print("EIG BASELINE CANDIDATE LIVE REPLAY SUMMARY " + json.dumps(report["summary"], sort_keys=True))
    if report["status"] != "passed":
        print("EIG BASELINE CANDIDATE LIVE REPLAY ERRORS " + json.dumps(report["validation_errors"], sort_keys=True))
        return 1
    print("EIG BASELINE CANDIDATE LIVE REPLAY PASS")
    return 0
