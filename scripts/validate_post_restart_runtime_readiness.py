#!/usr/bin/env python3
"""Validate post-restart runtime readiness for founder testing."""
...
from vllm_agent_gateway.acceptance.post_restart_runtime_readiness import (  # noqa: E402
    DEFAULT_POLICY_PATH,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-path", default="runtime-state/post-restart-runtime-readiness/phase163/phase163-post-restart-runtime-readiness-report.json")
    parser.add_argument("--health-drift-output-path", default="runtime-state/post-restart-runtime-readiness/phase163/phase163-health-drift-report.json")
    parser.add_argument("--doctor-output-path", default="runtime-state/post-restart-runtime-readiness/phase163/phase163-first-time-user-doctor.json")
    parser.add_argument("--session-recovery-output-path", default="runtime-state/post-restart-runtime-readiness/phase163/phase163-session-recovery-report.json")
...
def main() -> int:
    ...
    print("POST RESTART RUNTIME READINESS " + json.dumps(report["summary"], ensure_ascii=True, sort_keys=True))
    if report["status"] != "passed":
        print("POST RESTART RUNTIME READINESS ERRORS " + json.dumps(report["errors"], ensure_ascii=True, sort_keys=True))
        return 1
    print("POST RESTART RUNTIME READINESS PASS")
    return 0
