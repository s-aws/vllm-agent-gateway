#!/usr/bin/env python3
"""Validate Phase 208 evidence-quality live reruns."""
...
from vllm_agent_gateway.acceptance.evidence_quality_live_rerun import (  # noqa: E402
...
def main() -> int:
    ...
    report = validate_evidence_quality_live_rerun(
        EvidenceQualityLiveRerunConfig(
            ...
            target_roots=tuple(args.target_roots or ()),
            allow_partial=args.allow_partial,
            live=args.live,
        )
    )
    print("PHASE208 EVIDENCE QUALITY LIVE RERUN SUMMARY " + json.dumps(report.get("summary", {}), sort_keys=True))
    if report.get("status") == "preflight_passed":
        print("PHASE208 EVIDENCE QUALITY LIVE RERUN OFFLINE PREFLIGHT PASS")
        print("PHASE208 closeout requires --live without --allow-partial.")
        return 0
    if report.get("status") != "passed":
        print("PHASE208 EVIDENCE QUALITY LIVE RERUN FAILURES " + json.dumps(report.get("errors", []), sort_keys=True))
        failed = [
            {
                "surface": item.get("surface"),
                "case": item.get("live_case_id") or item.get("audit_case_id"),
                "baseline": item.get("audit_case_id"),
                "root": item.get("target_root"),
        ...
        if failed:
            print("PHASE208 FAILED RESPONSES " + json.dumps(failed, sort_keys=True))
        return 1
    print("PHASE208 EVIDENCE QUALITY LIVE RERUN PASS")
    return 0
