#!/usr/bin/env python3
"""Validate Phase 118 engineering-judgment blind-baseline prompt cases."""
...
DEFAULT_CATALOG_PATH = REPO_ROOT / "runtime" / "phase118_engineering_judgment_prompt_cases.json"
REQUIRED_CASE_TYPES = {
    "approach_tradeoff",
    "technical_debt_separation",
    "risk_blocker_summary",
    "review_feedback",
    "architecture_decision",
    "unsupported_preference_guard",
    "quick_patch_vs_debt",
    "implementation_decision_reasoning",
    "holdout_measurement_blocker",
    "holdout_missing_context",
}
REQUIRED_BASELINE_REQUIREMENTS = {
    ...
    "holdout_prompt_categories",
}
REQUIRED_JUDGMENT_TERMS = {
    "read only",
    "evidence",
    "tradeoff",
    "validation",
...
def validate_catalog(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    if catalog.get("kind") != "phase118_engineering_judgment_prompt_cases":
        issues.append(issue("catalog", "kind must be phase118_engineering_judgment_prompt_cases"))
    if catalog.get("schema_version") != 1:
        issues.append(issue("catalog", "schema_version must be 1"))
    if catalog.get("phase") != 118:
        issues.append(issue("catalog", "phase must be 118"))
    if catalog.get("priority_backlog_id") != "P0-BB-003":
    ...
    if len(cases) < 10:
        issues.append(issue("cases", "at least 10 Phase 118 cases are required"))
    ...
    for item in cases:
        ...
        if not isinstance(prompt, str) or len(prompt.split()) < 14:
            issues.append(issue(case_id, "prompt must be a natural-language request with at least 14 words"))
            continue
        ...
        missing_terms = sorted(term for term in REQUIRED_JUDGMENT_TERMS if term not in lowered)
        if missing_terms:
            issues.append(issue(case_id, f"prompt missing judgment terms: {missing_terms}"))
        if not any(term in lowered for term in ("alternative", "alternatives", "rejected", "unknown", "blocker", "debt", "risk", "risks")):
            issues.append(issue(case_id, "prompt must request alternatives, rejected reasons, unknowns, blockers, debt, or risks"))
    ...
    return {
        "kind": "phase118_engineering_judgment_prompt_case_report",
        "schema_version": 1,
...
def main() -> int:
    ...
    print("PHASE118 ENGINEERING JUDGMENT CASES " + json.dumps(report, ensure_ascii=True, sort_keys=True))
