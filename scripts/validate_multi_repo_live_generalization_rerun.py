#!/usr/bin/env python3
"""Validate Phase 212 multi-repo live generalization rerun."""
...
from vllm_agent_gateway.acceptance.multi_repo_live_generalization_rerun import (  # noqa: E402
...
def main() -> int:
    ...
    print("PHASE212 MULTI REPO LIVE GENERALIZATION RERUN SUMMARY " + json.dumps(report.get("summary", {}), sort_keys=True))
    if report.get("status") == "preflight_passed":
        print("PHASE212 MULTI REPO LIVE GENERALIZATION RERUN OFFLINE PREFLIGHT PASS")
        print("PHASE212 closeout requires --live without --allow-partial.")
        return 0
    if report.get("status") != "passed":
        print("PHASE212 MULTI REPO LIVE GENERALIZATION RERUN FAILURES " + json.dumps(report.get("errors", []), sort_keys=True))
        return 1
    print("PHASE212 MULTI REPO LIVE GENERALIZATION RERUN PASS")
    return 0
