#!/usr/bin/env python3
"""Collect Phase 117 defect-diagnosis local-model responses for blind-baseline comparison."""
...
DEFAULT_OUTPUT_PATH = "runtime-state/phase117/defect-diagnosis-local-eval.json"
...
def anythingllm_payload(case_id: str, prompt: str) -> dict[str, Any]:
    return {
        "message": prompt,
        "mode": "chat",
        "sessionId": f"phase117-defect-diagnosis-{case_id.lower()}-{uuid.uuid4().hex}",
    }
...
def response_diagnostics(text: str, baseline: dict[str, Any] | None) -> dict[str, Any]:
    ...
    diagnostics: dict[str, Any] = {
        ...
        "contains_likely_cause": "likely cause" in lowered or "root cause" in lowered,
        "contains_smallest_test": "smallest" in lowered and ("test" in lowered or "command" in lowered),
        "contains_broader_regression": "broad" in lowered or "regression" in lowered,
        "contains_observability": "observability" in lowered or "log" in lowered or "trace" in lowered,
        "contains_missing_data": "missing" in lowered or "gap" in lowered or "insufficient" in lowered,
    }
    if baseline:
...
def validate_port_health(timeout_seconds: int) -> list[dict[str, Any]]:
    ...
    for label, url in PORT_HEALTH_PROBES:
        ...
        print(f"PHASE117 PORT PASS label={label} url={url}")
    return checks
...
def collect_gateway_response(args: argparse.Namespace, case: dict[str, Any], baseline: dict[str, Any] | None) -> dict[str, Any]:
    ...
    print(f"PHASE117 GATEWAY CAPTURED case={case.get('case_id')} chars={len(text)}")
    return {
        "status": "captured",
        "http_status": status,
...
def collect_anythingllm_response(
    ...
    text = text_response(body)
    print(f"PHASE117 ANYTHINGLLM CAPTURED case={case_id} chars={len(text)}")
    return {
        "status": "captured",
        "http_status": status,
        "route_summary": route_summary(body, text),
...
def main() -> int:
    ...
    if not cases:
        raise RuntimeError("No Phase 117 cases selected")
    ...
    for case in cases:
        ...
        if not args.skip_anythingllm:
            ...
            validate_no_target_mutation(
                ...
                target_before[target_root],
                target_git_before[target_root],
                f"Phase 117 AnythingLLM {case_id}",
            )
    ...
    if runtime_changed:
        raise RuntimeError(f"canonical runtime metadata mutated during Phase 117 local eval: {runtime_changed}")

    report = {
        "kind": "phase117_defect_diagnosis_local_eval",
        "schema_version": 1,
        "status": "captured",
        "created_at": utc_now(),
    ...
    print(f"PHASE117 DEFECT DIAGNOSIS LOCAL EVAL CAPTURED report={output_path}")
    return 0
