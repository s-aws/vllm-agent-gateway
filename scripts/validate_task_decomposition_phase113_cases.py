#!/usr/bin/env python3
"""Validate the Phase 113 task-decomposition prompt and audit case catalog."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--catalog-path", default=str(DEFAULT_PHASE113_CASE_CATALOG))
    parser.add_argument("--output-path", default=None)
    parser.add_argument("--recursive-report-output-path", default=None)
    parser.add_argument(
        "--live-report-path",
        default="runtime-state/task-decomposition/phase113-live.json",
        help="Evidence reference for the Phase 113 live validation report.",
    )
    parser.add_argument(
        "--engineering-tenet-report-path",
        default="runtime-state/engineering-tenet-coverage/phase113-current.json",
        help="Evidence reference for the engineering-tenet coverage report.",
    )
    parser.add_argument(
        "--focused-regression-ref",
        default="python -m pytest tests/regression/test_task_decomposition.py -q -> 15 passed",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="runtime-state/recursive-blind-testing/phase113-task-decomposition-recursive-validation.json",
    )
...
def main() -> int:
    ...
    if args.recursive_report_output_path:
        ...
        recursive_report = build_phase113_recursive_blind_testing_report(
            catalog,
    ...
    print("TASK DECOMPOSITION PHASE113 CASES " + json.dumps(report, ensure_ascii=True, sort_keys=True))
