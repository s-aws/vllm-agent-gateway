#!/usr/bin/env python3
"""Validate Phase 275 clean-clone replay for the 500k candidate target."""
...
from vllm_agent_gateway.acceptance.large_context_500k_clean_clone_replay import (  # noqa: E402
...
def main() -> int:
    ...
    print(
        "PHASE275 LARGE CONTEXT 500K CLEAN CLONE REPLAY SUMMARY "
        + json.dumps(report.get("summary", {}), ensure_ascii=True, sort_keys=True)
    )
    if report["status"] != LargeContext500kCleanCloneReplayStatus.PASSED.value:
        print("PHASE275 LARGE CONTEXT 500K CLEAN CLONE REPLAY FAIL")
        print("PHASE275 ERRORS " + json.dumps(report.get("errors", []), ensure_ascii=True, sort_keys=True))
        return 1
    print("PHASE275 LARGE CONTEXT 500K CLEAN CLONE REPLAY PASS")
    return 0
