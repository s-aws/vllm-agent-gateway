#!/usr/bin/env python3
"""Validate Phase 152 AnythingLLM conversation state isolation."""
...
from vllm_agent_gateway.acceptance.anythingllm_conversation_state_isolation import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-path",
        default=(
            "runtime-state/anythingllm-conversation-state-isolation/phase152/"
            "phase152-anythingllm-conversation-state-isolation-report.json"
        ),
    )
    parser.add_argument(
        ...
        default=(
            "runtime-state/anythingllm-conversation-state-isolation/phase152/"
            "phase152-anythingllm-conversation-state-isolation-report.md"
        ),
    )
...
def main() -> int:
    ...
    print("ANYTHINGLLM CONVERSATION STATE ISOLATION SUMMARY " + json.dumps(report["summary"], ensure_ascii=True, sort_keys=True))
    if report["status"] != "passed":
        print("ANYTHINGLLM CONVERSATION STATE ISOLATION ERRORS " + json.dumps(report["errors"], ensure_ascii=True, sort_keys=True))
        return 1
    print("ANYTHINGLLM CONVERSATION STATE ISOLATION PASS")
    return 0
