#!/usr/bin/env python3
"""Collect Phase 118 engineering-judgment local-model responses for blind-baseline comparison."""
...
DEFAULT_OUTPUT_PATH = "runtime-state/phase118/engineering-judgment-local-eval.json"
...
def anythingllm_payload(case_id: str, prompt: str) -> dict[str, Any]:
    return {
        "message": prompt,
        "mode": "chat",
        "sessionId": f"phase118-engineering-judgment-{case_id.lower()}-{uuid.uuid4().hex}",
    }
...
def normalize_ref(value: str) -> str:
    ...
    return f"{match.group('path').lstrip('./')}{suffix}"
...
def evidence_refs_in_text(text: str) -> set[str]:
    ...
    for match in EVIDENCE_REF_RE.finditer(text):
        suffix = f":{match.group('line')}" if match.group("line") else ""
        refs.add(f"{match.group('path').lstrip('./')}{suffix}")
    return refs
...
def response_diagnostics(text: str, baseline: dict[str, Any] | None) -> dict[str, Any]:
    ...
    diagnostics: dict[str, Any] = {
        ...
        "contains_direct_recommendation": "recommendation" in lowered or "recommend " in lowered or "do not proceed" in lowered,
        "contains_tradeoffs": "tradeoff" in lowered or "benefit" in lowered or "cost" in lowered,
        "contains_risks": "risk" in lowered or "blocker" in lowered,
        "contains_validation": "validation" in lowered or "verify" in lowered or "test" in lowered,
        "contains_confidence": "confidence" in lowered,
        "contains_unknowns": "unknown" in lowered or "missing" in lowered or "insufficient" in lowered,
    }
    if baseline:
...
def validate_port_health(timeout_seconds: int) -> list[dict[str, Any]]:
    ...
    for label, url in PORT_HEALTH_PROBES:
        ...
        print(f"PHASE118 PORT PASS label={label} url={url}")
    return checks
...
def collect_gateway_response(args: argparse.Namespace, case: dict[str, Any], baseline: dict[str, Any] | None) -> dict[str, Any]:
    ...
    print(f"PHASE118 GATEWAY CAPTURED case={case.get('case_id')} chars={len(text)}")
    return {
        "status": "captured",
        "http_status": status,
...
def collect_anythingllm_response(
    ...
    text = text_response(body)
    print(f"PHASE118 ANYTHINGLLM CAPTURED case={case_id} chars={len(text)}")
    return {
        "status": "captured",
        "http_status": status,
        "route_summary": route_summary(body, text),
...
def main() -> int:
    ...
    if not cases:
        raise RuntimeError("No Phase 118 cases selected")
    ...
    for case in cases:
        ...
        if not args.skip_anythingllm:
            ...
            validate_no_target_mutation(
                ...
                target_before[target_root],
                target_git_before[target_root],
                f"Phase 118 AnythingLLM {case_id}",
            )
    ...
    if runtime_changed:
        raise RuntimeError(f"canonical runtime metadata mutated during Phase 118 local eval: {runtime_changed}")

    report = {
        "kind": "phase118_engineering_judgment_local_eval",
        "schema_version": 1,
        "status": "captured",
        "created_at": utc_now(),
    ...
    print(f"PHASE118 ENGINEERING JUDGMENT LOCAL EVAL CAPTURED report={output_path}")
    return 0
